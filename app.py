...
if run_btn:
    ...
    selected_period_for_dist = st.selectbox(
        "選擇要查看的持有期",
        options=period_cols,
        index=min(3, len(period_cols) - 1),
        key="dist_period"
    )

    dist_data = valid_df[selected_period_for_dist].dropna()
    ...
    # 垂直線
    fig_dist.add_vline(x=mu,  line_color="#facc15", line_dash="dash", line_width=2,
        annotation_text=f"μ={mu:+.1f}%", annotation_font_color="#facc15",
        annotation_position="top right")
    fig_dist.add_vline(x=0, line_color="#6b7280", line_dash="dot", line_width=1.5,
    ...
    for val, lbl, pos in [
        ...
        fig_dist.add_vline(x=val, line_color="#a78bfa", line_dash="dot", line_width=1,
    ...
    # 解讀文字
    p_positive = (dist_data > 0).mean() * 100
    st.markdown(f"""
    <div class="insight-box">
    <b>📐 {selected_period_for_dist} 統計解讀（共 {n} 筆）</b><br><br>
    平均報酬 μ = <b>{mu:+.1f}%</b>&nbsp;&nbsp;｜&nbsp;&nbsp;標準差 σ = <b>{sigma:.1f}%</b><br><br>
    🔵 <b>±1σ 區間（約68%機率）</b>：報酬落在 <b>{mu-sigma:+.1f}% ～ {mu+sigma:+.1f}%</b><br>
    🔷 <b>±2σ 區間（約95%機率）</b>：報酬落在 <b>{mu-2*sigma:+.1f}% ～ {mu+2*sigma:+.1f}%</b><br><br>
